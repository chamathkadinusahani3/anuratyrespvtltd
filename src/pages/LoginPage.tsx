...
export function LoginPage() {
  ...
  const getFriendlyError = (code: string) => {
    const map: Record<string, string> = {
      'auth/user-not-found':       'No account found with this email. Did you mean to register?',
    ...
    <div className="min-h-screen flex bg-neutral-950 text-white overflow-hidden">
      ...
      <div className="hidden lg:flex lg:w-[45%] relative overflow-hidden flex-col items-center justify-center p-16 bg-black">
        ...
        <div className="absolute inset-0 opacity-[0.04]"
          style={{
            backgroundImage: `repeating-linear-gradient(45deg, #FFD700 0px, #FFD700 1px, transparent 0px, transparent 50%)`,
        ...
        >
          ...
          <p className="text-neutral-300 text-lg leading-relaxed font-light italic">
            "{TYRE_TAGLINES[taglineIdx]}"
          </p>

          {/* Branch count pills */}
          <div className="flex flex-wrap justify-center gap-2 mt-10">
            {['Pannipitiya', 'Ratnapura', 'Kalawana', 'Nivithigala'].map(b => (
              <span key={b} className="px-3 py-1 rounded-full text-xs border border-white/10 text-neutral-500 bg-white/5">
                {b}
              </span>
            ))}
          </div>
      ...
      <div className="w-full lg:w-[55%] flex flex-col justify-center px-6 sm:px-12 lg:px-20 py-12 relative">
        ...
        >
          ...
          <AnimatePresence>
            ...
            {resetSent && (
              <motion.div
                ...
                animate={{ opacity: 1, height: 'auto' }}
                exit={{ opacity: 0, height: 0 }}
                className="mb-5 px-4 py-3 rounded-xl bg-green-500/10 border border-green-500/25 text-green-400 text-sm"
              >
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button
                  ...
                  className="text-xs text-brand-yellow hover:text-yellow-300 transition-colors"
                >
                  Forgot password?
                </button>
              </div>
          ...
          <p className="text-center text-sm text-neutral-600 mt-8">
            ...
            <Link to="/register" className="font-bold text-brand-yellow hover:text-yellow-300 transition-colors">
              Create one free →
            </Link>
          </p>

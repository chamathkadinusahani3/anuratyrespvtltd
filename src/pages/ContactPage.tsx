...
import { Phone, Mail, MessageCircle, MapPin, AlertCircle, CheckCircle, X } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { useActivityTracker } from '../hooks/useActivityTracker';

export function ContactPage() {
  const { track } = useActivityTracker({ type: 'page_view', page: '/contact' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        track({ type: 'contact_form', item: 'submitted', page: '/contact' });
      } else {
            ...
            <div className="lg:col-span-2">
              <Card className="p-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-brand-gray mb-1.5">Message *</label>
                    <textarea
                      name="message"
                      rows={5}
...
function StatusModal({ status, onClose }: { status: 'success' | 'error'; onClose: () => void }) {
  return (
    <motion.div
      ...
      >
        ...
        {status === 'success' ? (
          <>
            ...
            <p className="text-gray-400 text-sm">One of our call center operators will contact you shortly. Thank you!</p>
          </>
        ) : (
          <>
